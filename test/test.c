...
void print_kv(void *k, void *v) {
    printf("%s: %s, ", (char *) k, (char *) v);
}

test_func(array)
    coll_array_t array;
    coll_array_init(&array, 3);
...
test_func(heap)
    coll_heap_t heap;
    coll_heap_init(&heap, coll_cmp_str);
...
test_func(hmap)
    coll_hmap_t map;
    coll_hmap_init_custom(&map, 0, coll_hash_str, coll_equal_str);
...
test_func(tmap)
    coll_tmap_t map;
    coll_tmap_init_custom(&map, coll_cmp_str);
